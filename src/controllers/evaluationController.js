...
exports.getExamStats = async (req, res) => {
    try {
        ...
        if (idAnnee) {
            ...
            const ids = trimestres.map(t => t.idTrimes);
            if (ids.length > 0) where.idTrimestre = { [Op.in]: ids };
        }

        const total = await Epreuve.count({ where });
        const publiees = await Epreuve.count({ where: { ...where, is_published: 1 } });
        const nonPubliees = await Epreuve.count({ where: { ...where, is_published: 0 } });

        const parNature = await Epreuve.findAll({
            where,
            attributes: [
                'idNature',
                [sequelize.fn('COUNT', sequelize.col('idEpreuve')), 'count']
            ],
            include: [{ model: NatureEpreuve, as: 'nature', attributes: ['libelle', 'code'] }],
            group: ['idNature'],
            raw: true
        });

        const parMatiere = await Epreuve.findAll({
            where,
            attributes: [
                'idMatiere',
                [sequelize.fn('COUNT', sequelize.col('idEpreuve')), 'count']
            ],
            include: [{ model: Matiere, as: 'matiere', attributes: ['libelle'] }],
            group: ['idMatiere'],
            raw: true
        });
        ...
        console.error('[examController.getExamStats]', err);
        res.status(500).json({ success: false, message: err.message });
    }
};
